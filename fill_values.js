...
const STAT_MANUAL = {
    ...
    //   ★ 초가스 E — 가시가 앞으로 뻗는 거리가 실제로 있다. bin castRange 40 은 기본 공격 강화 쪽 값이라
    //     예전엔 STAT_DROP 으로 뺐는데, **투사체 속도 1475 는 적어 놓고 사거리만 없는 모양**이라 앞뒤가 안 맞았다.
    '초가스 E': { '사거리': '650' },   // 롤위키 `650 (From an offset 25 units in front of Cho'Gath)`
    //   ★★ 트위치 R — 우리 1200 은 **E(`TwitchExpunge`) 와 같은 값**이었다. R 은 자기 강화 스킬이라
    //     시전 사거리가 없고(위키 `target range = Twitch's attack range`), 1100 은 **강화된 평타 화살이
    //     관통하며 날아가는 거리**다. 공격 사거리가 늘면 같이 늘어난다.
    '트위치 R': { '사거리': '1100 (+ 추가 공격 사거리)' },   // 롤위키 `1100 (+ 100% bonus attack range)` = Bolt missile travel range
...
const STAT_DROP = {
    ...
    // ★ 초가스 E 사거리는 2026-08-24 에 되살렸다 (STAT_MANUAL 650) — 인게임에서 가시가 앞으로 뻗는 거리가 실제로 있다.
    //   bin castRange 40 은 기본 공격 강화 쪽 값이라 쓸 수 없었던 것뿐이다
