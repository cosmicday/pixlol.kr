...
// 생성 시각: 2026-08-08T08:23:48.514Z
...
const customTemplates = {
    ...
    "Vex": { // 벡스
        ...
        "R": "그림자가 격렬하게 전방으로 돌진합니다. 그림자는 <magicdamage>{p1}의 마법 피해</magicdamage>를 입히고 첫 번째로 적중한 적 챔피언에게 4초 동안 표식을 남깁니다.<br><br><recast>재사용 시</recast>: 표식이 남은 챔피언에게 돌진하여 도착 시 <magicdamage>{p2}의 마법 피해</magicdamage>를 입힙니다.<br><br>표식이 남은 챔피언이 이 스킬로 피해를 입은 뒤 {p3}초 이내에 사망할 경우, 이 스킬의 재사용 대기시간이 일시적으로 초기화됩니다.", // 그림자 파동
    },
    "Belveth": { // 벨베스
    ...
    "Smolder": { // 스몰더
        "P": "스킬로 챔피언을 맞히고 <spellname>초강력 화염 숨결</spellname> 스킬로 적을 처치하면 <spellname>용 훈련</spellname> 중첩을 1회 얻습니다. 중첩은 스몰더의 기본 스킬을 강화합니다.<br><li><spellname>초강력 화염 숨결</spellname>: <magicdamage>{p1}의 마법 피해</magicdamage>를 추가로 입히고 {p2}회, {p3}회, {p4}회 중첩 시 새로운 효과를 얻습니다.<li><spellname>에취!</spellname>: 폭발이 <magicdamage>{p5}의 마법 피해</magicdamage>를 추가로 입힙니다.<li><spellname>펄럭펄럭</spellname>: 적중할 때마다 <magicdamage>{p6}의 마법 피해</magicdamage>를 추가로 입히고 중첩이 {p7}회 쌓일 때마다 추가 번개를 얻습니다.<br><br><rules>스몰더가 <font color='#c93c3c'>용 훈련</font> 중첩을 <font color='#c93c3c'>{p8}</font> 획득했습니다.</rules>", // 용 훈련 — stringtable
        "Q": "스몰더가 불꽃을 내뿜어 <physicaldamage>{p1}의 물리 피해</physicaldamage>+<magicdamage>{p2}의 마법 피해</magicdamage>를 입힙니다. 대상이 사망하면 스몰더가 스킬 사용 한 번당 <scalemana>{p3}의 마나</scalemana>를 돌려받습니다.<br><br>이 스킬은 <spellname>용 훈련</spellname> 중첩 수에 따라 진화하며 다음과 같은 효과를 얻습니다.<li><keywordmajor>{p4}회 중첩</keywordmajor>: 대상 주변의 모든 적에게 피해를 입힙니다.<li><keywordmajor>{p5}회 중첩</keywordmajor>: 이 스킬 피해량의 {p6}%만큼 피해를 입히는 폭발 <spellname>{p7}</spellname>개를 대상 너머로 날립니다.<li><keywordmajor>{p8}회 중첩</keywordmajor>: 대상을 불태우며 {p9}초에 걸쳐 <truedamage>최대 체력의 {p10}에 해당하는 고정 피해</truedamage>를 입힙니다. 불타는 동안 총 체력이 <truedamage>{p11}</truedamage> 밑으로 떨어지는 적 챔피언은 즉시 처치됩니다.", // 초강력 화염 숨결
        "W": "스몰더가 앙증맞은 불꽃 재채기를 내뿜어 <physicaldamage>{p1}의 물리 피해</physicaldamage>를 입히고 {p2}초 동안 {p3}% <status>둔화</status>시킵니다.<br><br>챔피언에게 적중하면 폭발이 일어나며 <physicaldamage>{p4}의 물리 피해</physicaldamage>+<magicdamage>{p5}의 마법 피해</magicdamage>를 입힙니다.", // 에취!
        "E": "스몰더가 하늘을 날며 {p1}초 동안 <speed>이동 속도가 {p2}%</speed> 증가하고 지형을 무시합니다.<br><br>스몰더는 비행하는 동안 체력이 가장 낮은 적을 <spellname>{p3}</spellname>(내림 적용)회 폭격해 공격 적중 시 <physicaldamage>{p4}의 물리 피해</physicaldamage>+<magicdamage>{p5}의 마법 피해</magicdamage>를 입힙니다.", // 펄럭펄럭
    ...
    "Aphelios": { // 아펠리오스
        "P": "아펠리오스는 쌍둥이 누이 알룬이 만든 다섯 가지의 루나리 무기를 사용합니다. 한 번에 주 무기와 보조 무기 등 총 두 가지 무기를 사용할 수 있습니다. 각 무기는 고유의 기본 공격 및 [{p1}] 스킬을 가지고 있습니다. 기본 공격과 스킬 사용 시 탄약을 소모하며, 탄약을 모두 소모하면 사용 중인 주 무기를 다음 무기로 교체합니다. <br><br>2레벨 도달 시 [{p1}] 스킬을 사용할 수 있으며 6레벨 도달 시 궁극기를 사용할 수 있습니다. 아펠리오스는 스킬 포인트로 스킬 레벨을 올리는 대신 영구 능력치를 획득합니다.<br><br><passive>루나리 무기 순서:</passive><li>{{ Spell_ApheliosP_GunShortDesc_{p2} }}</li><li>{{ Spell_ApheliosP_GunShortDesc_{p3} }}</li><font color='#424242'><li>{{ Spell_ApheliosP_GunShortDesc_{p4} }}</li><li>{{ Spell_ApheliosP_GunShortDesc_{p5} }}</li><li>{{ Spell_ApheliosP_GunShortDesc_{p6} }}</li></font>", // 암살자와 예언자 — stringtable
        "Q": "", // 무기 스킬
    ...
    "Kaisa": { // 카이사
        "P": "기본 공격 시 {p1}초 동안 <keywordmajor>플라즈마</keywordmajor> 중첩을 쌓고 <magicdamage>중첩당 {p2}+{p3}의 마법 피해</magicdamage>를 추가로 입힙니다. {p4}회 중첩이 쌓인 적을 공격하면 <keywordmajor>플라즈마</keywordmajor>를 소모하며, <magicdamage>대상이 잃은 체력의 {p5}에 해당하는 마법 피해</magicdamage>를 입힙니다. 주변 아군이 적 챔피언을 <status>이동 불가</status> 상태로 만들면 중첩을 {p6}회 적용합니다.<br><br>카이사의 피부가 능력치에 따라 기본 스킬을 진화시킵니다.<br><spellname>이케시아 폭우</spellname>: <physicaldamage>추가 공격력 {p7}/{p8}</physicaldamage><br><spellname>공허추적자</spellname>: <scaleap>주문력 {p9}/{p10}</scaleap><br><spellname>고속 충전</spellname>: <attackspeed>추가 공격 속도 {p11}%/{p12}%</attackspeed>", // 두 번째 피부 — stringtable
    ...
    "Khazix": { // 카직스
        ...
        "R": "<active>사용 시:</active> 카직스가 {p1}초 동안 <keywordstealth>투명</keywordstealth> 상태가 되고 <speed>이동 속도가 {p2}%</speed> 상승합니다. {p3}초 안에 이 스킬을 <recast>재사용</recast>할 수 있습니다.<br><br><passive>기본 지속 효과:</passive> 이 스킬을 레벨 업하면 <evolve>진화</evolve>를 통해 스킬 하나에 추가 효과를 부여합니다.<li><spellname>공포 감지:</spellname> 스킬 및 기본 공격 사거리가 늘어나고 <keywordmajor>고립</keywordmajor>된 대상에게 사용 시 재사용 대기시간이 {p4}% 감소합니다.<li><spellname>공허의 가시:</spellname> 가시를 세 개 발사하고 적을 {p5}% <status>둔화</status>합니다. <keywordmajor>고립</keywordmajor>된 대상에게는 효과가 증가합니다.<li><spellname>도약:</spellname> 사거리가 늘어나고 챔피언 처치 관여 시 재사용 대기시간이 초기화됩니다.<li><spellname>공허의 습격:</spellname> {p6}초 동안 <keywordstealth>투명</keywordstealth> 상태가 되고 2회 <recast>재사용</recast>할 수 있습니다.", // 공허의 습격
    },
    "Katarina": { // 카타리나
    ...
    "Kayle": { // 케일
        ...
        "E": "<passive>기본 지속 효과:</passive> 기본 공격이 <magicdamage>{p1}의 마법 피해</magicdamage>를 추가로 입힙니다.<br><br><active>사용 시:</active> 케일의 다음 공격 사거리가 증가하며 <magicdamage>대상이 잃은 체력의 {p2}만큼 마법 피해</magicdamage>를 추가로 입힙니다. 이 공격은 케일이 <scalelevel>{p3}레벨</scalelevel>에 도달하면 대상에게 적중 시 폭발하여 주변 적에게 피해를 입힙니다.", // 화염주문검
