...
## ★★ 2026-09-17 저녁 개편 — 리그별 일정 · 2단 배치 · 다시보기 · 대진 결과

**사용자가 「대회별 경기 일정 정확도」를 봐 달라고 해서 재 보니 리그 탭이 잘려 있었다.**

### 무엇이 틀렸었나 — 합산 페이지 80경기 = 최근 3주

`getSchedule` 은 리그를 몇 개 묻든 **한 페이지 80경기**다. 11개를 콤마로 한 번에 물으면 그게 최근 3주고,
리그 칩은 그 80개에서 걸러 보여 주고 있었다. 실측(9/17):

| 탭 | 합산에서 거른 것 | 리그별로 따로 물으면 |
|---|---|---|
| LCK | 13경기 (플레이-인부터) | 80경기 (5/17~9/13 · 그룹 스테이지 포함) |
| LCP | 2경기 | 80경기 |
| 월즈·MSI·퍼스트 스탠드·케스파컵·EWC | **0 (빈 화면)** | 63 · 26 · 38 · 28 |

**고친 것**: 리그를 고르면 `?league=lck` 로 **그 리그만 따로** 받는다 (`esportsLeagueData[slug]`, 서버 캐시 5분 / 창고 `schedule_<slug>`).
「전체」는 지금처럼 합산 피드(최근 3주)다 — 리그 11개를 다 따로 받아 합치면 264KB 라 안 했다.
리그별 응답이 실패하면 옛 방식(합산에서 거르기)으로 버티고 「최근 3주만」 안내를 띄운다 (`esportsEventsFor`).

### 같이 넣은 것

- **다시보기** — `match.flags` 의 `hasVod` 를 `vod` 로 넘긴다. 완료 경기 카드는 `lolesports.com/ko-KR/vod/<경기id>/1` 로 간다
  (307 로 한국어 VOD 로 이어진다 — 실측. **추가 요청 0**). `/ko-KR/match/<id>` 는 404 다
- **대진 결과(`brackets`)** — `rankings` 가 없는 스테이지(플레이-인·플레이오프·지역별 챔피언십)의 `matches` 를 결과 목록으로.
  **양 팀이 다 TBD 인 경기는 뺀다** (월즈 2026 은 46경기 전부 TBD 라 통째로 비고, 그러면 「다음 대회」 안내가 나온다)
- **리그 로고** — `getLeagues` 의 `image` 를 하루 한 번 받아 칩·리그 머리글에 붙인다 (`esportsLeaguesWithImg`)
- **스테이지 칩** — 그 리그 일정의 `blockName` 을 시간순으로 모아 거른다 (`7주 차 … 플레이-인 · 플레이오프`). 주소는 안 바꾼다
- **2단 배치** — ≥1024 에서 왼쪽 일정 · 오른쪽 순위표+대진 결과(sticky). **sticky top 은 공통 헤더 높이를 화면이 재서 `--es-top` 에 넣는다** —
  헤더가 sticky 라 0 이면 밑에 깔린다. 날짜 머리글도 같은 값으로 sticky (배경이 그림이라 반투명 판)
- 순위표에 **승률 열 + 막대**, 1위 강조. 「예정된 경기 없음」 은 큰 상자 대신 한 줄 (`#es-next`, 다음 대회 기간이 여기 채워진다)
- 진행 중 경기 LIVE 점 깜빡임 · 「예정된 경기」 옆 진행 중 개수 · 오늘 날짜 강조

### ★ 이번에 걸린 함정

- **★★ 헤드리스로 여러 페이지를 연달아 찍으면 `apiLimiter`(분당 30) 에 걸려 「불러오지 못했습니다」 가 찍힌다.**
  폰 두 장이 그렇게 나와서 한참 봤다 — API 는 멀쩡했다. 스크린샷 사이에 60초 쉬거나 한 번에 4장 이하로
- 카드 메타 칸(132px)에 「플레이오프 · BO5 · 다시보기」 를 한 줄로 넣으면 잘린다 → 다시보기는 리그 배지 옆 첫 줄로
- 폰 스테이지 칩은 가로 스크롤이라 넘침 검사에 `es-stage` 가 잡힌다 — `scrollWidth` 가 390 이면 페이지는 안 넘친 것

### 안 한 것

- 브래킷 **그림**(트리) — 결과 목록으로 대신했다. `previousMatchIds` 가 오므로 다음에 그릴 수 있다
- 리그 추가(EMEA Masters · CBLOL · VCS · LJL 은 일정이 있다. 아시안게임은 리그만 있고 대회·일정이 비어 있다) — 사용자 결정 대기
