...
const KEEP_TEXT = {
    ...
    Draven: ['P', 'R_rules'],
    //   블라디미르 P — "추가 주문력: X / 추가 체력: Y" 두 줄을 지웠다 (2026-08-12).
    //   드레이븐 P 와 같은 성격이다. 앞의 두 문장("추가 체력 30당 주문력 1",
    //   "주문력 1당 최대 체력 1.6")이 패시브의 **실제 규칙**이고, 지운 두 줄은
    //   그 규칙을 **지금 내 스탯에 적용한 결과**라 고정값이 없다.
    //   식이 서로를 참조해서(체력→주문력→체력) 화면에 `(추가 최대 체력의 100% +
    //   1.6 x 주문력의 -100%) x 0.037` 이라는 날 계산식이 그대로 찍히고 있었다.
    Vladimir: ['P'],
    //   ★ 아크샨 W — 본문이 `{{ Spell_AkshanW_Tooltip_{p1} }}` **한 줄뿐**이었다 (2026-08-13).
    //   키 안에 `{pN}` 이 또 있어서 정적 조회가 안 되고, 이 형태는 폴백도 안 걸려
    //   **화면에 `{{ Spell_AkshanW_Tooltip_1 }}` 이라는 글자가 그대로 찍히고 있었다.**
    //   `{p1}` 은 `GameModeInteger` 이고 값이 1(협곡)이라 stringtable 의
    //   `spell_akshanw_tooltip_1` 을 손으로 풀어 넣었다. `_2` 는 다른 모드 판본이라
    //   **아군 부활이 빠져 있다** — 협곡 아크샨 W 의 핵심이므로 `_1` 이 맞다.
    //   값 6개는 bin 에서 뽑아 롤위키와 대조했다 (이동 속도 80/90/100/110/120 등 전부 일치).
    //   ★ 다른 KEEP_TEXT 자리와 같이 **숫자를 문장에 직접 박았다.** 값 쪽 `pN` 은
    //     CD 원본 `desc` 기준이라 `p1` 하나뿐이어서 `{p2}` 를 쓸 수가 없다.
    Akshan: ['W', 'W_rules'],
    //   ★ 아래 넷은 전부 `<stattracking>` — 인게임에서 실시간으로 세는 누적값이라
    //   고정값이 없다 (드레이븐 P 와 같은 성격). 값이 `?` 라서 회색 글씨가 통째로
    //   안 나가고 있었다. 최대값이 같이 적힌 자리는 현재값만 지우고 최대값은 살렸다.
    //     드레이븐 R — "리그 소용돌이 처형: {p6}" 누적 횟수라 줄을 지웠다
    //     아우렐리온 솔 R — "모은 별가루 {p8}/{p4}" -> "필요한 별가루 {p4}"
    //     초가스 R — "현재 중첩 {p5}/{p4}" -> "최대 {p4}까지 쌓입니다" 로 정보를 살렸다
    //     파이크 R — 두 줄 다 누적 골드뿐이라 남길 게 없어 통째로 비웠다
    Chogath: ['R_rules'], Pyke: ['R_rules'],
};
